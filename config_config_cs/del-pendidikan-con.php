<?php
	if(isset($_GET['id'])) {
		...
		$id 	= $_GET['id'];
		$p_nip  = $_GET['nip'];
		$cek = mysqli_query($connect, "SELECT * FROM tbl_detail_pendidikan WHERE id='$id'") or die(mysqli_error($connect));

		if(mysqli_num_rows($cek) == 0) {
			echo '<script>window.history.back()</script>';
		} else {
			$q_delete_pendidikan = mysqli_query($connect, "DELETE FROM tbl_detail_pendidikan WHERE id = '$id'");
			if ($q_delete_pendidikan) { ?>
